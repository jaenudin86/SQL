...
FROM Person
	...
	--**************************************************************
	-- ATTEMPTS AT SOLVING QUESTION 3
	--**************************************************************
/* FOURTH PASS
--This is a section of code to check if the grant exists
	IF EXISTS --Check to see if grant exists (checking to see if the person exists was already accomplished with the code above)
		(SELECT sg.GrantAmount, sg.GrantDate, sg.GrantNeedExplanation
			FROM dbo.ServiceGrant sg 
			WHERE @PersonKey =
			--IN
				(SELECT p.PersonKey FROM Person p JOIN
				(PersonAddress pa JOIN PersonContact pc 
				...
				AND p.PersonFirstName = @userFirst
				AND pc.ContactInfo = @userContact)
			AND GrantAmount = @userGrantAmount
			AND GrantDate = @userGrantDate
			AND GrantNeedExplanation = @userGrantNeedExplanation)
		--BEGIN --Number 2
			PRINT 'Grant already in database'
...
IF EXISTS
	...
	AND p.PersonFirstName = @userFirst
	AND pc.ContactInfo = @userContact)
	BEGIN-- Number 2 Begin. This displays a message that the person exists
		...
		AND p.PersonFirstName = @userFirst
		AND pc.ContactInfo = @userContact)
		IF NOT EXISTS --If the person exists but the grant does not, try to insert the grant
			(SELECT GrantAmount, GrantDate, PersonKey, GrantNeedExplanation
			FROM dbo.ServiceGrant
			WHERE PersonKey = @PersonKey
			AND GrantAmount = @userGrantAmount
			AND GrantDate = @userGrantDate
			AND GrantNeedExplanation = @userGrantNeedExplanation)
			BEGIN --Number 1 BEGIN
				BEGIN TRY	--Try to insert user grant request
					INSERT INTO ServiceGrant(GrantAmount, GrantDate, PersonKey, GrantNeedExplanation)
					VALUES(@userGrantAmount, @userGrantDate, @PersonKey, @userGrantNeedExplanation)
					PRINT 'The new grant request is now inserted successfully.'
					RETURN
				END TRY --End insert if successful.  If not successful, go to CATCH
				BEGIN CATCH --BEGIN CATCH takes the place of an ELSE statement to IF NOT EXISTS (which was implemented several lines above).  If the grant already exists, display a message stating so.
					PRINT 'The grant request is already in the database.'
					RETURN
				END CATCH 
			END --Number 1 END. End the block of code that checks to see if the grant exists
	PRINT 'The person already exists.'
	RETURN
	END--Number 2 End
ELSE
BEGIN TRAN --If the person does not exist (as vetted by the code above), insert new person info AND new grant request
	BEGIN TRY
		--DECLARE @PersonKey INT
...
/*--SECOND PASS
IF EXISTS --Check to see if the person exists, and then if the grant exists
	--Check to see if person exists
	(SELECT p.PersonLastName, p.PersonFirstName, pa.Street, pa.Apartment, pa.City, pa.State, pa.Zip, pc.ContactInfo
	FROM Person p JOIN
		(ServiceGrant sg  JOIN 
			(PersonContact pc JOIN PersonAddress pa 
			ON pc.PersonKey = pa.PersonKey)
		ON sg.PersonKey = pc.PersonKey)
	ON p.PersonKey = sg.PersonKey
	WHERE p.PersonLastName = @userLast
	AND p.PersonFirstName = @userFirst
	AND pc.ContactInfo = @userContact)
	BEGIN -- NUMBER 1 BEGIN Declare @PersonKey = PersonKey from database
		DECLARE @PersonKey1 INT
		SELECT @PersonKey1 = p.PersonKey
		FROM Person p JOIN
		(ServiceGrant sg  JOIN 
			(PersonContact pc JOIN PersonAddress pa 
			ON pc.PersonKey = pa.PersonKey)
...
IF NOT EXISTS --If the person exists but the grant does not, try to insert the grant
	(SELECT GrantAmount, GrantDate, PersonKey, GrantNeedExplanation
	FROM dbo.ServiceGrant
	WHERE PersonKey = @PersonKey1
	AND GrantAmount = @userGrantAmount
	AND GrantDate = @userGrantDate
	AND GrantNeedExplanation = @userGrantNeedExplanation)
	BEGIN --NUMBER 2 BEGIN
	BEGIN TRY	--Try to insert user grant request
		INSERT INTO ServiceGrant(GrantAmount, GrantDate, PersonKey, GrantNeedExplanation)
		VALUES(@userGrantAmount, @userGrantDate, @PersonKey1, @userGrantNeedExplanation)
	RETURN
	END TRY --End insert if successful.  If not successful, go to CATCH
	BEGIN CATCH --If, by some chance, the grant already exists, display a message stating so
	PRINT 'The grant request exists'
	RETURN
	END CATCH 
	END --NUMBER 2 END End the block of code that checks to see if the grant exists
ELSE
	BEGIN  -- NUMBER 3 BEGIN If the person AND the grant already exist, display a message and end
	PRINT 'The person and grant request are already in the database'
	RETURN
	END --NUMBER 3 END  
BEGIN TRAN --BEGIN TRAN does NOT need an END.
BEGIN TRY
		DECLARE @PersonKey INT
		SET @PersonKey=IDENT_CURRENT('Person')
		-- Insert user personal info
		INSERT INTO dbo.Person(PersonLastName,PersonFirstName)
		VALUES(@userLast, @userFirst)
		INSERT INTO dbo.PersonAddress(Street, Apartment, City, State, Zip, PersonKey)
		VALUES(@userStreet, @userApartment, @userCity, @userState, @userZip, @PersonKey)
		INSERT INTO dbo.PersonContact(ContactInfo, PersonKey)
		VALUES(@userContact, @PersonKey)
		--Insert user grant request
		INSERT INTO dbo.ServiceGrant(GrantAmount, GrantDate, PersonKey, GrantNeedExplanation)
		VALUES(@userGrantAmount, @userGrantDate, @PersonKey, @userGrantNeedExplanation)
		--Commit transaction
		COMMIT TRAN
END TRY
BEGIN CATCH
	...
	PRINT error_message()
END CATCH;
END -- NUMBER 1 END Ends block of code that declares @PersonKey = PersonKey, and encapsulates all the code checking whether or not a grant request already exists, and if not, inserting the new grant request.
GO
*/
